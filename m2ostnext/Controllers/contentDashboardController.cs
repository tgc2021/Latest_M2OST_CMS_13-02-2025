...
namespace m2ostnext.Controllers
{
    ...
    public class contentDashboardController : Controller
    {
        ...
        [HttpPost]
        public ActionResult UplodeContent(List<ProcessedData> data, string linkParent, string contentExpiry,string selectedCategory,string hidcategory)
        {
            try
            {
                ...
                foreach (var item in data)
                {
                    ...
                    //string str1 = null; 
                    //string str2 = this.Request.Form["btn_submit"].ToString(); // save and exite
                    string str2 = "0";
                    ...
                    string str1 = null;
                    string str3 = "0";
                    int int32_2 =Convert.ToInt32(item.Theme);
                    List<string> stringList = new List<string>();
                    int int32_3 = 1;
                    DateTime now = DateTime.Now;
                    DateTime datetime = new Utility().StringToDatetime(contentExpiry);
                    int content_id = 0;
                    int num2 = 0;
                    ...
                    try
                    {
                        entity2.CONTENT_HEADER = item.ContentHeader;
                        entity2.CONTENT_QUESTION =item.ContentQuestion;
                        entity2.CONTENT_TITLE = item.ContentTitle;
                        ...
                        if (content_id > 0)
                        {
                            int int32_5 = Convert.ToInt32(hidcategory);
                            for (int index = 1; index <= int32_5; ++index)
                            {
                                string str5 =selectedCategory;
                        ...
                        entity3.CONTENT_ANSWER1 = item.ContentAnswer;
                        ...
                        //Console.WriteLine($"Processing: {item.ContentTitle}, {item.ContentAnswer}");
                    }
                    catch (Exception ex)
                    {
        ...
        public ActionResult UplodeCreation()
        {
            UserSession userSession = (UserSession)System.Web.HttpContext.Current.Session["UserSession"];
            if (userSession != null)
            {
        ...
        public ActionResult createContent()
        {
          ...
          (object) int32_1
            ...
            if (int32_2 == 27)
            {
                if (this.Request.Form["select-theme-level"] != null)
                {
                   
                    if (this.Request.Form["select-theme-level"] != null)
                    {
                        int32_2_T = this.Request.Form["select-theme-level"].ToString();
                        int32_2 = 1;
                    }
                }
                else
            ...
            try
            {
                ...
                switch (int32_2)
                {
                    case 1:
                        ...
                        else if(int32_2_T == "17")
                        {
                            entity3.CONTENT_ANSWER_IMG1 = this.Request.Form["t-content"].ToString();
                        }
        ...
        public ActionResult updateContent()
        {
          ...
          (object) content_id
                ...
                if (idTheme == 1)
                {
                    ...
                    if (((IEnumerable<string>)System.Web.HttpContext.Current.Request.Files.AllKeys).Any<string>())
                    {
                        HttpPostedFile file1 = System.Web.HttpContext.Current.Request.Files["Video_path"];
                        HttpPostedFile file2 = System.Web.HttpContext.Current.Request.Files["Video_path"];
                        if (file2 != null && file1.ContentLength > 0)
                        {
                            ...
                            string directoryPath = this.HttpContext.Server.MapPath("~/Content/SKILLMUNI_DATA/" + content1.id_ORGANIZATION + "/" + content_id + "/");
                            if (!Directory.Exists(directoryPath))
                                Directory.CreateDirectory(directoryPath);

                            // Delete the previous file if it exists
                            string previousFile = Path.Combine(directoryPath, tblContentAnswer.CONTENT_ANSWER_IMG1);
                            ...
                            // Save the new file
                            string filename = Path.Combine(directoryPath, content_id.ToString() + "-1" + extension);
                            file1.SaveAs(filename);
                            tblContentAnswer.CONTENT_ANSWER_IMG1 = content_id.ToString() + "-1" + extension;

                        }
                        else if (int32_2_T == "text")
                        {
                            tblContentAnswer.CONTENT_ANSWER_IMG1 = "";

                        }
                        else if (int32_2_T == "url")
                        {
                            tblContentAnswer.CONTENT_ANSWER_IMG1 = this.Request.Form["YoutubeUrl"].ToString();

                        }
                        else if (int32_2_T == "gif")
                        {
                            HttpPostedFile file1_gif = System.Web.HttpContext.Current.Request.Files["Gif_path"];
                            HttpPostedFile file2_gif = System.Web.HttpContext.Current.Request.Files["Gif_path"];
                            if (file2_gif != null && file1_gif.ContentLength > 0)
                            {
                                string extension = Path.GetExtension(file2_gif.FileName);
                                if (!Directory.Exists(this.HttpContext.Server.MapPath("~/Content/SKILLMUNI_DATA/" + content1.id_ORGANIZATION + "/" + (object)content_id + "/")))
                                    Directory.CreateDirectory(this.HttpContext.Server.MapPath("~/Content/SKILLMUNI_DATA/" + content1.id_ORGANIZATION + "/" + (object)content_id + "/"));
                                string filename = Path.Combine(this.HttpContext.Server.MapPath("~/Content/SKILLMUNI_DATA/" + content1.id_ORGANIZATION + "/" + (object)content_id + "/"), content_id.ToString() + "-1" + extension);
                                file1_gif.SaveAs(filename);
                                tblContentAnswer.CONTENT_ANSWER_IMG1 = content_id.ToString() + "-1" + extension;
                            }

                        }
                        else if (int32_2_T == "image")
                        {
                            HttpPostedFile file1_gif = System.Web.HttpContext.Current.Request.Files["Image_path"];
                            HttpPostedFile file2_gif = System.Web.HttpContext.Current.Request.Files["Image_path"];
                            if (file2_gif != null && file1_gif.ContentLength > 0)
                            {
                                string extension = Path.GetExtension(file2_gif.FileName);
                                if (!Directory.Exists(this.HttpContext.Server.MapPath("~/Content/SKILLMUNI_DATA/" + content1.id_ORGANIZATION + "/" + (object)content_id + "/")))
            ...
            for (int i = 0; i < totalUsers; i += batchSize)
            {
                ...
                foreach (tbl_user tblUser in batch)
                {
                    ...
                    if (contentProgramMapping == null)
                    {
                        ...
                        //            try
                        //            {
                        //                System.Data.DataTable dataTable = ConvertToDataTable(entities1); // Convert List to DataTable
                        //                List<string> valueList = new List<string>();

                        //                foreach (DataRow row in dataTable.Rows)
                        //                {
                        //                    string startDate = row["start_date"] != DBNull.Value ? Convert.ToDateTime(row["start_date"]).ToString("yyyy-MM-dd HH:mm:ss") : "NULL";
                        //                    string expiryDate = row["expiry_date"] != DBNull.Value ? Convert.ToDateTime(row["expiry_date"]).ToString("yyyy-MM-dd HH:mm:ss") : "NULL";
                        //                    string updatedDateTime = row["updated_date_time"] != DBNull.Value ? Convert.ToDateTime(row["updated_date_time"]).ToString("yyyy-MM-dd HH:mm:ss") : "NULL";

                        //                    valueList.Add($"({row["map_type"]}, {row["id_role"]}, {row["id_user"]}, {row["id_organization"]}, {row["id_category"]}, '{row["status"]}', {row["option_type"]}, '{startDate}', '{expiryDate}', {row["id_category_tile"]}, {row["id_category_heading"]}, {row["id_assessment_sheet"]}, '{updatedDateTime}')");

                        //                    // When the batch size is reached, execute the query
                        //                    if (valueList.Count >= batchSize)
                        //                    {
                        //                        string bulkInsertQuery = "INSERT INTO tbl_content_program_mapping (map_type, id_role, id_user, id_organization, id_category, status, option_type, start_date, expiry_date, id_category_tile, id_category_heading, id_assessment_sheet, updated_date_time) VALUES " + string.Join(", ", valueList);
                        //                        using (MySqlCommand command = new MySqlCommand(bulkInsertQuery, connection, transaction))
                        //                        {
                        //                            command.ExecuteNonQuery();
                        //                        }
                        //                        valueList.Clear(); // Clear the batch after insertion
                        //                    }
                        //                }
                        ...
                        //}
                        //entities1.Clear(); 
                        // Clear entities1 for next batch
        ...
        /// <summary>
        private System.Data.DataTable ConvertToDataTable(List<tbl_content_program_mapping> entities1)
        {
            System.Data.DataTable table = new System.Data.DataTable();
            table.Columns.Add("map_type", typeof(int));
            table.Columns.Add("id_role", typeof(int));
            table.Columns.Add("id_user", typeof(int));
            table.Columns.Add("id_organization", typeof(int));
            table.Columns.Add("id_category", typeof(int));
            table.Columns.Add("status", typeof(string));
            table.Columns.Add("option_type", typeof(int));
            table.Columns.Add("start_date", typeof(string));
            table.Columns.Add("expiry_date", typeof(string));
            table.Columns.Add("id_category_tile", typeof(int));
            table.Columns.Add("id_category_heading", typeof(int));
            table.Columns.Add("id_assessment_sheet", typeof(int));
            table.Columns.Add("updated_date_time", typeof(string));

            foreach (var entity in entities1)
            {
                string startDate = entity.start_date.HasValue ? entity.start_date.Value.ToString("yyyy-MM-dd HH:mm:ss") : DBNull.Value.ToString();
                string expiryDate = entity.expiry_date.HasValue ? entity.expiry_date.Value.ToString("yyyy-MM-dd HH:mm:ss") : DBNull.Value.ToString();
                string updatedDateTime = entity.updated_date_time.HasValue ? entity.updated_date_time.Value.ToString("yyyy-MM-dd HH:mm:ss") : DBNull.Value.ToString();

                table.Rows.Add(
                    entity.map_type.HasValue ? (object)entity.map_type.Value : DBNull.Value,
                    entity.id_role.HasValue ? (object)entity.id_role.Value : DBNull.Value,
                    entity.id_user.HasValue ? (object)entity.id_user.Value : DBNull.Value,
                    entity.id_organization.HasValue ? (object)entity.id_organization.Value : DBNull.Value,
                    entity.id_category.HasValue ? (object)entity.id_category.Value : DBNull.Value,
                    entity.status ?? "A",  // No need to check HasValue for strings
                    entity.option_type.HasValue ? (object)entity.option_type.Value : DBNull.Value,
                    startDate,  // Use formatted DateTime string
                    expiryDate,  // Use formatted DateTime string
                    entity.id_category_tile.HasValue ? (object)entity.id_category_tile.Value : DBNull.Value,
                    entity.id_category_heading.HasValue ? (object)entity.id_category_heading.Value : DBNull.Value,
                    entity.id_assessment_sheet.HasValue ? (object)entity.id_assessment_sheet.Value : DBNull.Value,
                    updatedDateTime  // Use formatted DateTime string
                );
