...
namespace m2ostnext.Controllers
{
    ...
    public class AssessmentController : Controller
    {
        ...
        public ActionResult add_assessment()
        {
            ...
            this.db.SaveChanges();
            int id_assessment = entity1.id_assessment;
            int? certificatePercentage = null; // Default to null

            if (!string.IsNullOrEmpty(this.Request.Form["certificate_percentage"]))
            {
                certificatePercentage = Convert.ToInt32(this.Request.Form["certificate_percentage"]);
            }
            using (MySqlConnection conn = new MySqlConnection(ConfigurationManager.ConnectionStrings["dbconnectionstring"].ConnectionString))
            {
                ...
                {
                    conn.Open();
                    string query = "UPDATE tbl_assessment SET certificate_percentage = @certificate_percentage WHERE id_assessment = @id_assessment";

                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@certificate_percentage", certificatePercentage);
                        cmd.Parameters.AddWithValue("@id_assessment", id_assessment);
                        ...
                        {
                            Console.WriteLine("No record updated. Check the id_assessment value.");
                        }
                    }
                }
                catch (Exception ex)
